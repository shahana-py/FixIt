...
//         }
//
//         // Fetch ratings for this specific service and provider
//         String serviceId = doc.id;
//         String providerId = data['provider_id'] ?? '';
//         double averageRating = await _fetchAverageRating(serviceId, providerId);
//
//         services.add({
//           'id': doc.id,
//           'name': data['name'] ?? '',
...
//   }
//
// // Function to fetch and calculate the average rating for a service provider
//   Future<double> _fetchAverageRating(String serviceId, String providerId) async {
//     try {
//       // Query ratings collection for the specific service and provider
...
//   @override
//   void dispose() {
//     _notificationSubscription?.cancel(); // Clean up subscription
//     super.dispose();
//   }
//
//
...
//         // 2. It's specifically for this user
...
//       if (currentUser == null) return;
//
//       // Get all unread notifications that apply to this user
...
//           .get();
//
//       // Filter to only mark applicable ones as read
...
//                       isFavorite: false,
//                       isApproved: false, // Default to false for dummy cards
//                       onTap: () {
...
//                       isApproved: service['isApproved'] ?? false, // Pass the approval status
//                       onTap: () async {
...
//     required bool isApproved, // This parameter isn't being used correctly
...
//     _fetchServices();
//     _fetchUserFavorites();
//     _determinePosition().then((_) => _fetchNearbyServices());
//     _startAutoScroll();
//   }
//
//   @override
...
//         String address = provider['address'] ?? ''; // Assuming the address field is named 'address'
//
...
//       List<String> nearbyProviderIds = nearbyProviders.map((p) => p['id'] as String).toList();
//       QuerySnapshot servicesSnapshot = await FirebaseFirestore.instance
//           .collection('services')
//           .where('provider_id', whereIn: nearbyProviderIds)
//           .limit(6)
//           .get();
//
...
//         var provider = nearbyProviders.firstWhere(
//               (p) => p['id'] == service['provider_id'],
//           orElse: () => {},
//         );
//
//         double averageRating = await _fetchAverageRating(doc.id, service['provider_id']);
//
//         nearbyServices.add({
//           'id': doc.id,
//           'name': service['name'] ?? '',
...
//             'id': '1',
//             'name': 'Plumbing',
//             'image': 'https://imageapi.ralfiz.com/media/ProductImages/scaled_1000572194.jpg',
//             'icon': 'https://imageapi.ralfiz.com/media/ProductImages/scaled_1000572199.jpg'
//           },
//           {
//             'id': '2',
//             'name': 'Taxi service',
//             'image': 'https://imageapi.ralfiz.com/media/ProductImages/scaled_1000595027.jpg',
//             'icon': 'https://imageapi.ralfiz.com/media/ProductImages/scaled_1000595028.jpg'
//           },
//         ];
//       });
//     }
...
//             context,
//             MaterialPageRoute(
//               builder: (context) => ViewServiceDetailsPage(serviceId: serviceId),
//             ),
//           );
//           if (result == true) {
//             setState(() {});
...
//   Timer? _offersTimer;
//   StreamSubscription<
//       QuerySnapshot<Map<String, dynamic>>>? _notificationSubscription;
//
//   @override
//   void initState() {
//     super.initState();
...
//
//       // Get services from nearby providers
//       List<String> nearbyProviderIds = nearbyProviders.map((
//           p) => p['id'] as String).toList();
//       QuerySnapshot servicesSnapshot = await FirebaseFirestore.instance
//           .collection('services')
//           .where('provider_id', whereIn: nearbyProviderIds)
//           .limit(6)
...
//         );
//
//         double averageRating = await _fetchAverageRating(
//             doc.id, service['provider_id']);
//
//         nearbyServices.add({
//           'id': doc.id,
//           'name': service['name'] ?? '',
...
//           'isApproved': provider['status'] == 1,
//           'distance': provider['distance']?.toStringAsFixed(1) ?? 'Unknown',
//           'isActive': service['isActive'] ?? true, // Add isActive field
//         });
//       }
//
//       setState(() {
...
//   }
//
//   Future<double> _fetchAverageRating(String serviceId,
...
//                   padding: const EdgeInsets.only(left: 15),
//                   child: CircleAvatar(
//                     backgroundColor: _profileImageUrl == null ? Colors
//                         .yellowAccent[600] : null,
//                     backgroundImage: _profileImageUrl != null
//                         ? NetworkImage(_profileImageUrl!)
//                         : null,
//                     child: _profileImageUrl == null
...
//                         ScaffoldMessenger.of(context).showSnackBar(
//                           SnackBar(
//                               content: Text('No service details available')),
//                         );
//                       },
//                     );
//                   } else {
