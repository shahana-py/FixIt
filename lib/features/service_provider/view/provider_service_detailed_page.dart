...
//     var doc = await FirebaseFirestore.instance.collection('services').doc(widget.serviceId).get();
//     if (doc.exists) {
//       setState(() {
...
//     final TextEditingController nameController = TextEditingController(text: serviceData?['name']);
//     final TextEditingController descriptionController = TextEditingController(text: serviceData?['description']);
//     final TextEditingController experienceController = TextEditingController(text: serviceData?['experience'].toString());
//     final TextEditingController rateController = TextEditingController(text: serviceData?['hourly_rate'].toString());
//
//     List<String> selectedAreas = List<String>.from(serviceData?['available_areas'] ?? []);
//     List<String> selectedDays = List<String>.from(serviceData?['available_days'] ?? []);
//
...
//             return AlertDialog(
//               title: Text("Edit Service", style: TextStyle(color: Color(0xff0F3966))),
...
//                     _buildTextField(experienceController, "Experience (Years)", inputType: TextInputType.number),
//                     _buildTextField(rateController, "Hourly Rate", inputType: TextInputType.number),
//                     SizedBox(height: 16),
//
...
//                     : selectedItems.join(', '),
//                 style: TextStyle(
//                   color: selectedItems.isEmpty ? Colors.grey : Colors.black87,
//                 ),
...
//             color: selectedItems.contains(day) ? Color(0xff0F3966) : Colors.black87,
//             fontWeight: selectedItems.contains(day) ? FontWeight.bold : FontWeight.normal,
//           ),
//           shape: RoundedRectangleBorder(
//             borderRadius: BorderRadius.circular(8),
//             side: BorderSide(
//               color: selectedItems.contains(day) ? Color(0xff0F3966) : Colors.grey[300]!,
//             ),
//           ),
//         );
//       }).toList(),
...
//                     SizedBox(height: 12),
//                     _buildDetailRow("Experience:", "${serviceData!['experience']} years"),
//                     Divider(height: 20, color: Colors.grey[300]),
//                     _buildDetailRow("Hourly Rate:", "₹${serviceData!['hourly_rate']}"),
//                     Divider(height: 20, color: Colors.grey[300]),
...
//                               SizedBox(height: 8),
//                               Text("No samples added yet", style: TextStyle(color: Colors.grey[600])),
//                             ],
//                           ),
//                         ),
//                       ),
...
//
//
// import '../../../core/shared/services/image_service.dart'; // Import the ImageService
//
// class ServiceDetailsPage extends StatefulWidget {
//   final String serviceId;
//
...
//       setState(() {
//         serviceData = doc.data()!;
//         // Ensure work_samples is always a List, handling both array and single string cases
//         if (serviceData!.containsKey('work_sample') && !serviceData!.containsKey('work_samples')) {
//           // If there's only a single work_sample string, convert it to a list
//           if (serviceData!['work_sample'] is String) {
//             serviceData!['work_samples'] = [serviceData!['work_sample']];
//           }
//         } else if (!serviceData!.containsKey('work_samples')) {
//           // Initialize an empty list if no work samples exist
//           serviceData!['work_samples'] = [];
//         }
//       });
//     }
//   }
...
//             // Also update work_sample field for backward compatibility
//             'work_sample': downloadUrl
//           });
//
//           await _loadServiceData(); // Refresh UI
//           ScaffoldMessenger.of(context).showSnackBar(
...
//         text: serviceData?['experience']?.toString() ?? '0');
//     final TextEditingController rateController = TextEditingController(
//         text: serviceData?['hourly_rate']?.toString() ?? '0');
//
//     List<String> selectedAreas = List<String>.from(serviceData?['available_areas'] ?? []);
//     List<String> selectedDays = List<String>.from(serviceData?['available_days'] ?? []);
//
...
//                         _buildSectionHeader("Work Samples"),
//                         ElevatedButton.icon(
//                           onPressed: _isUploading ? null : _pickAndUploadSampleImage,
//                           icon: _isUploading
//                               ? SizedBox(
//                             width: 18,
//                             height: 18,
...
//                         itemBuilder: (context, index) {
//                           String url = serviceData!['work_samples'][index];
//                           // Ensure the URL is processed through ApiConstants if needed
//                           if (!url.startsWith('http')) {
//                             url = _imageService.getFullImageUrlFromApi(url);
//                           }
//
//                           return ClipRRect(
//                             borderRadius: BorderRadius.circular(8),
...
//             .update({
//           'work_samples': currentSamples,
//           // If we're removing the last image or the main work_sample, update it
//           if (currentSamples.isEmpty)
//             'work_sample': ''
//           else if (serviceData!['work_sample'] == url)
//             'work_sample': currentSamples.first
//         });
//
...
//           }
//         } else {
//           // If using your custom API, try to delete it there
//           try {
//             await _imageService.deleteImage(url);
//           } catch (e) {
//             print("Error deleting from custom API: $e");
//           }
//         }
//
//         await _loadServiceData();
